...
public class WebCrawler implements Runnable {
	...
	/**
	 * This function is called once the header of a page is fetched. It can be overridden by sub-classes to perform custom logic for different status
	...
	/**
	 * Classes that extends WebCrawler should overwrite this function to tell the crawler whether the given url should be crawled or not. The
	 * following default implementation indicates that all urls should be included in the crawl except those with a nofollow flag.
	 *
	 * @param url
	...
	public boolean shouldVisit(Page referringPage, WebURL url) {
		if (myController.getConfig().isRespectNoFollow()) {
			return !((referringPage != null && referringPage.getContentType() != null && referringPage.getContentType().contains("html") && ((HtmlParseData) referringPage
					.getParseData()).getMetaTagValue("robots").contains("nofollow")) || url.getAttribute("rel").contains("nofollow"));
		}
	...
	/**
	 * Determine whether links found at the given URL should be added to the queue for crawling. By default this method returns true always, but
	 * classes that extend WebCrawler can override it in order to implement particular policies about which pages should be mined for outgoing links
	 * and which should not.
	 *
	 * If links from the URL are not being followed, then we are not operating as a web crawler and need not check robots.txt before fetching the
	 * single URL. (see definition at http://www.robotstxt.org/faq/what.html). Thus URLs that return false from this method will not be subject to
		...
		try {
			...
			if (statusCode < 200 || statusCode > 299) { // Not 2XX: 2XX status codes indicate success
				if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY || statusCode == HttpStatus.SC_MOVED_TEMPORARILY
					...
					String contentType = fetchResult.getEntity() == null ? "" : fetchResult.getEntity().getContentType() == null ? "" : fetchResult
				...
				if (page.isTruncated()) {
					logger.warn("Warning: unknown page size exceeded max-download-size, truncated to: " + "({}), at URL: {}", myController
							.getConfig().getMaxDownloadSize(), curURL.getURL());
				}
	...
	private void parseOutgoingWebUrls(Page page) {
		Set<WebURL> outgoingWebUrls = new HashSet<>();
		Set<String> outgoingUrls = getOutgoingUrls(page);

		if (outgoingUrls != null) {
			outgoingWebUrls = outgoingUrls.stream().filter(url -> url != null && url.length() > 0).map(url -> {
				WebURL webURL = new WebURL();
		...
		page.getParseData().setOutgoingUrls(outgoingWebUrls);
	}

	public Set<String> getOutgoingUrls(Page page) {
		return Net.extractUrls(((HtmlParseData) page.getParseData()).getHtml());
	}
